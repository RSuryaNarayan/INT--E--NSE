...
if (prob_type==3)
    ...
    breadth = 1;%length along the positive y-direction of the flow domain
    mesh_x = 301; %number of cells discretized along the x-direction
    mesh_y = 60; %number of cells discretized along the y-direction
end
...
beta=1; %blending factor for controlling interpolation in convection terms
outer_iterations = 150; %number of times SIMPLE is going to be iterated through for convergence
...
for k = 1:outer_iterations
    ...
    apu=ae+aw+as+an+mn-ms+me-mw;
    apu = apu/omega_u;
    if (iblock==1)
        apu(25:35,100:111)=1e30;
    end
        ...
        for j = 2:mesh_x
            for i = 2:mesh_y+1
                u(i,j) = (1-omega_u)*u_old(i,j) + (1/apu(i,j))*...
                    (ae(i,j)*u(i,j+1) + aw(i,j)*u(i,j-1) + an(i,j)*u(i+1,j)+ as(i,j)*u(i-1,j) -beta*(dcc(i,j)-dcu(i,j))+ dy*(pressure(i,j)-pressure(i,j+1)));
            end
        end
        if (prob_type==1)
    ...
    apv=ae+aw+as+an+me-mw+mn-ms;
    %iterative solver
    apv = apv/omega_v;
    if (iblock==1)
        apv(25:36,100:110)=1e30;
    end
    for var = 1:iter_v
        for j = 2:mesh_x+1
            for i = 2:mesh_y
                v(i,j) = v_old(i,j)*(1-omega_v) + (1/apv(i,j))*...
                    (ae(i,j)*v(i,j+1) + aw(i,j)*v(i,j-1) + as(i,j)*v(i-1,j)+ an(i,j)*v(i+1,j) -beta*(dcc(i,j)-dcu(i,j))+ dx*(pressure(i,j)-pressure(i+1,j)));
            end
        end
    end
    %% RESIDUALS MONITOR DISPLAY
