...
if (prob_type==3)
    ...
    breadth = 1;%length along the positive y-direction of the flow domain
    mesh_x = 501; %number of cells discretized along the x-direction
    mesh_y = 60; %number of cells discretized along the y-direction
end
...
outer_iterations = 6; %number of times we are going to iterate thru continuity, X and Y momentum
number_time_steps=100;%number of time steps
...
if (prob_type==1)
    ...
    u(:,1) = 10; %entry velocity is 1 m/s
...
master_X=[];master_Y=[];master_U=[];master_V=[];
%time loop
...
%     end
    if (rem(time,1)==0)
        ...
        figure(1);
        hold on;
        contourf(X,Y,U,'ShowText','off','LineColor','none','LevelStep',0.1);
        colormap(jet);
%         quiver(X,Y,U,V,5,'Color','k','LineWidth',0.5);
        title('X-magnitude Contours for Channel flow');
    end
end
